      ...
      <nav className="mx-auto flex h-[72px] sm:h-20 max-w-[1600px] items-center justify-between px-5 sm:px-8 lg:px-12 xl:px-16">
        ...
        <div className="hidden items-center gap-10 md:flex">
          {[['/about', 'About'], ['/products', 'Products'], ['/contact', 'Contact']].map(([path, label]) => (
            <Link key={path} to={path} className={`relative py-2 text-xs font-bold uppercase tracking-wider transition-colors ${active(path) ? 'text-[#EF2034]' : 'text-[#104360]/80 hover:text-[#104360]'}`}>
              {label}
              <span className={`absolute bottom-0 left-0 h-px bg-[#EF2034] transition-all ${active(path) ? 'w-full' : 'w-0'}`} />
            </Link>
          ))}
          <Link to="/contact" className="group flex items-center gap-3 border-l border-[#104360]/10 pl-10 text-xs font-bold uppercase tracking-wider text-[#104360]">
            Enquire
        ...
        <div className="flex items-center gap-2 md:hidden">
          <Link to="/contact" className="bg-[#EF2034] px-4 py-2 text-xs font-bold uppercase tracking-wider text-white rounded-xs">Enquire</Link>
      ...
      {open && (
        <div className="fixed inset-0 top-[74px] z-40 flex flex-col bg-[#F7F5F0] px-6 py-8 md:hidden">
          <div className="mb-8 flex items-center justify-between border-b border-[#104360]/10 pb-4">
            <span className="text-xs font-bold uppercase tracking-wider text-[#EF2034]">Navigation</span>
            <span className="font-mono text-xs font-semibold text-[#104360]/60">BF / 01</span>
          </div>
          <div className="flex flex-col">
            {[['/', 'Home'], ['/about', 'About Boven Frontier'], ['/products', 'Products & Brands'], ['/#markets', 'Export Markets'], ['/#faq', 'Wholesale FAQ'], ['/contact', 'B2B Enquiries']].map(([to, label], i) => (
              <Link key={`${to}-${label}`} to={to} className="group flex items-center justify-between border-b border-[#104360]/10 py-5 text-xl font-medium tracking-tight text-[#104360]">
                <span><span className="mr-4 font-mono text-xs font-bold text-[#EF2034]">0{i + 1}</span>{label}</span>
                <ArrowUpRight size={18} className="opacity-40 transition group-hover:translate-x-1 group-hover:-translate-y-1 group-hover:opacity-100" />
              </Link>
            ))}
          </div>
          <div className="mt-auto grid grid-cols-2 gap-2 border-t border-[#104360]/10 pt-5">
            <a href={`https://wa.me/${whatsapp}`} target="_blank" rel="noopener noreferrer" className="border border-[#104360]/20 py-3 text-center text-xs font-bold uppercase tracking-wider text-[#104360]">WhatsApp</a>
            <a href={`tel:${phone}`} className="bg-[#104360] py-3 text-center text-xs font-bold uppercase tracking-wider text-white">Call Direct</a>
          </div>
        </div>
      )}
    </header>
