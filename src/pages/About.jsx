    ...
    <main className="bg-[#F7F5F0] text-[#104360]">
      <section className="relative overflow-hidden bg-[#104360] text-white">
        ...
        <div className="mx-auto grid min-h-[68vh] max-w-[1600px] items-end gap-10 px-5 py-16 sm:px-10 lg:grid-cols-12 lg:px-16 lg:py-24">
          <div className="relative z-10 lg:col-span-9">
            <div className="mb-8 flex items-center gap-4 text-xs font-bold uppercase tracking-wider text-white/80">
          ...
          <div className="relative z-10 lg:col-span-3 lg:pb-2">
            <p className="border-l border-[#EF2034] pl-5 text-sm sm:text-base leading-relaxed text-white/85">
        ...
        <div className="mx-auto flex max-w-[1600px] justify-between border-t border-white/10 px-5 py-4 text-xs uppercase tracking-wider text-white/65 sm:px-10 lg:px-16">
      ...
      <section className="mx-auto max-w-[1600px] px-5 py-24 sm:px-10 lg:px-16 lg:py-36">
        <div className="grid gap-14 lg:grid-cols-12">
          <div className="lg:col-span-4">
            <p className="text-xs font-bold uppercase tracking-wider text-[#EF2034]">01 — The company</p>
          ...
          <div className="lg:col-span-7 lg:col-start-6">
            <p className="text-xl leading-8 tracking-tight text-[#104360]/90 sm:text-2xl font-normal">
            ...
            <p className="mt-8 max-w-2xl text-sm sm:text-base leading-relaxed text-gray-600">
            ...
            <Link to="/contact" className="group mt-10 inline-flex items-center gap-4 text-xs font-bold uppercase tracking-wider text-[#104360]">
      ...
      <section className="border-y border-[#104360]/10 bg-white">
        <div className="mx-auto max-w-[1600px] px-5 py-20 sm:px-10 lg:px-16 lg:py-28">
          <div className="mb-14 flex items-end justify-between gap-6">
            <div><p className="text-xs font-bold uppercase tracking-wider text-[#EF2034]">02 — How we work</p><h2 className="mt-4 text-4xl font-medium tracking-[-0.05em] sm:text-6xl">Four things we don't compromise.</h2></div>
            <span className="hidden font-mono text-xs font-semibold text-[#104360]/60 sm:block">BF / 2026</span>
          </div>
          <div className="grid border-l border-t border-[#104360]/10 sm:grid-cols-2 lg:grid-cols-4">
            {pillars.map(([number, Icon, title, desc]) => (
              <article key={number} className="group min-h-[300px] border-b border-r border-[#104360]/10 p-7 transition hover:bg-[#104360] hover:text-white sm:p-9">
                <div className="flex items-center justify-between"><span className="font-mono text-xs font-bold text-[#EF2034]">{number}</span><Icon size={20} strokeWidth={1.4} className="text-[#104360]/40 transition group-hover:text-white/70" /></div>
                <h3 className="mt-20 text-xl font-bold tracking-tight">{title}</h3>
                <p className="mt-3 text-xs sm:text-sm leading-relaxed text-gray-600 transition group-hover:text-white/85">{desc}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="bg-[#104360] text-white">
        <div className="mx-auto grid max-w-[1600px] gap-12 px-5 py-24 sm:px-10 lg:grid-cols-12 lg:px-16 lg:py-32">
          <div className="lg:col-span-7"><p className="text-xs font-bold uppercase tracking-wider text-[#EF2034]">03 — Company credentials</p><h2 className="mt-5 text-4xl font-medium tracking-[-0.05em] sm:text-6xl">A real company behind every shipment.</h2></div>
          <div className="lg:col-span-4 lg:col-start-9">
            <dl className="divide-y divide-white/10 border-y border-white/10 text-xs sm:text-sm">
              {[['Legal entity', company.name], ['LLP registration', company.llpId], ['GSTIN', company.gstin], ['Origin', company.origin || 'India'], ['Office', company.address]].map(([label, value]) => <div key={label} className="grid grid-cols-2 gap-4 py-4"><dt className="uppercase tracking-wider font-semibold text-white/65">{label}</dt><dd className="text-right text-white/90 font-medium">{value}</dd></div>)}
            </dl>
          </div>
        </div>
      </section>
